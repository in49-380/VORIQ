...
public record ApiWrapper(Function<Service, RequestSpecification> specProvider) {
    ...
    /* =========================
       ЕДИНЫЙ ИСПОЛНИТЕЛЬ ЗАПРОСОВ
       ========================= */
    private ValidatableResponse request(
            Service svc,
            String method,                 // GET/POST/PUT/DELETE…
            String path,
            Object body,                   // может быть null
            Map<String, ?> queryParams,    // может быть null
            int delay,                     // может быть 0
            int expectedStatus,
            ContentType expectedContentType // может быть null (тогда не проверяем)
    ) {
