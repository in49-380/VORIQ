...
-- Purpose: Ensure there are up to N (default 500) rows in cars_car by generating combinations
--          across existing models, engines, market, transmissions, wheel drives, and years.
--
-- Controls:
--   GUC: voriq.seed_target_cars_car (int) — optional; if set, overrides default 500.
--
-- Notes:
--   - This script assumes the following tables/columns exist (names as in current schema):
--       cars_carmodel(id), cars_engine(id), cars_market(id),
--       cars_transmission(id), cars_whilldrive(id), cars_year(id, "year"),
--       cars_car(id, model_id, engine_id, market_id, transmission_id, whill_drive_id, year_id).
--   - If cars_car.id is GENERATED ALWAYS AS IDENTITY, remove the explicit id column/values.
...
WITH
  ...
  existing AS (
    SELECT COUNT(*) AS cnt FROM cars_car
  ),
  need AS (
  ...
  mk AS (SELECT id FROM cars_market ORDER BY id LIMIT 1),
...
INSERT INTO cars_car
  (id, model_id, engine_id, market_id, transmission_id, whill_drive_id, year_id)
SELECT
  CASE
    WHEN (SELECT s FROM seqname) IS NOT NULL
