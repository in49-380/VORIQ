...
@DisplayName("Fuel type controller integration tests: ")
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
@DisplayNameGeneration(value = DisplayNameGenerator.ReplaceUnderscores.class)
class FuelTypeControllerIT {
    ...
    @Nested
    @DisplayName("GET: /api" + FUEL_TYPE_URL)
    class getAllTests {

        @Test
        @Order(1)
        public void get_all_fuel_types_should_return_200() throws Exception {

            MvcResult result = mockMvc.perform(get(FUEL_TYPE_URL)
                            .header(HttpHeaders.AUTHORIZATION, "Bearer " + tmpToken1))
                    .andExpect(status().isOk())
                    .andReturn();

            String responseBody = result.getResponse().getContentAsString();
            ItemResponseDto<String> dto = mapper.readValue(responseBody, FuelTypeResponseDto.class);
        ...
        public void get_all_fuel_types_when_empty_should_return_empty_list() throws Exception {
            mockMvc.perform(get(FUEL_TYPE_URL)
                            .header(HttpHeaders.AUTHORIZATION, "Bearer " + tmpToken1))
                    .andExpect(status().isOk())
        ...
        @Test
        @Order(3)
        public void get_all_fuel_types_should_return_401_token_is_incorrect() throws Exception {
            MvcResult result = mockMvc.perform(get(FUEL_TYPE_URL)
                            .header(HttpHeaders.AUTHORIZATION, "Bearer " + "wrong_token"))
                    .andExpect(status().isUnauthorized())
                    .andReturn();

            checkErrorResponseResult(result, HttpStatus.UNAUTHORIZED, FUEL_TYPE_URL);
        }

        @Test
        @Order(4)
        public void get_all_fuel_types_should_return_401_header_authorization_is_null() throws Exception {
            MvcResult result = mockMvc.perform(get(FUEL_TYPE_URL))
                    .andExpect(status().isUnauthorized())
                    .andReturn();
        ...
        @Test
        @Order(5)
        public void get_all_fuel_types_should_return_401_header_authorization_is_not_bearer() throws Exception {
            MvcResult result = mockMvc.perform(get(FUEL_TYPE_URL)
        ...
        @Test
        @Order(6)
        void get_all_fuel_types_should_return_500_when_service_throws_exception() throws Exception {
            try {
                doThrow(new RuntimeException("Temporary service error."))
                        .when(fuelTypeServiceImp).getAll();
                ...
                checkErrorResponseResult(result, HttpStatus.INTERNAL_SERVER_ERROR, FUEL_TYPE_URL);
            } finally {
                reset(fuelTypeServiceImp);
            }
        }

        @Test
        @Order(7)
        void get_all_fuel_types_should_return_503_when_database_unavailable() throws Exception {
            try {
                doThrow(new RuntimeException("DB down"))
                        .when(fuelTypeJdbcRepository).findAll();
                ...
                checkErrorResponseResult(result, HttpStatus.SERVICE_UNAVAILABLE, FUEL_TYPE_URL);
            } finally {
                reset(fuelTypeJdbcRepository);
            }
        }
    }
}