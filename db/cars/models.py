...
# Search Result model - Model for storing search results
class SearchResult(models.Model):
    """
    Model for storing search query results. Connects the search query with found cars. 
    Contains found links and rating for each result.
    ...
    # Text field for storing found links (URLs separated by newlines or commas)
    found_links = models.TextField(blank=True, verbose_name="Found Links", help_text="URLs of found car listings, separated by newlines")
    
    # Integer field for storing rating (0-100 or 1-5 scale, depending on requirements)
    rating = models.IntegerField(default=0, verbose_name="Rating", help_text="Rating of the search result (0-100)")
    
    # Date and time of record creation.
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Found At")

    def __str__(self):
        return f"Result for Query #{self.search_query.id}: {self.car} (Rating: {self.rating})"
